...
const PagesNavigation: FC<PagesNavigationProps> = ({ pagesCount, currentPage }) => {
    ...
    let pagesNumbers = pagesCount
                ...
                value: currentPage + i + (currentPage < 5 ? 1 - currentPage : -4) + (pagesCount - currentPage > 4 ? 0 : -4 - currentPage + pagesCount),
                key: i
            })
        )
                       : undefined;
    pagesNumbers = pagesNumbers
                   ? pagesNumbers.slice((currentPage > 5 || pagesCount! - currentPage < 4) ? 2 : 0,
            ...
            {pagesCount && pagesNumbers &&
                <>
                    ...
                    {pagesNumbers.map((p, i) =>
                        <Link key={p.key} className={p.value === currentPage ? styles.disabled : undefined} to={`/page/${p.value}`} children={p.value} />)
                    }
